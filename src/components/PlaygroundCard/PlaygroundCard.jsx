...
export default function PlaygroundCard({ title, description, href }) {
  return (
    <div className={styles["playground-card"]}>
      <Link href={href}>
        <div className={styles["title-container"]}>
          <h2 className={`${styles.first} ${styles.centered}`}>{title}</h2>
          <h2 className={`${styles.second} ${styles.centered}`}>{title}</h2>
        </div>
        <p className={styles.description}>{description}</p>
        <div className={styles["view-link"]}>
          <p className={styles.button}>view functions</p>
          <svg
